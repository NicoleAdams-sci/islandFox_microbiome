...
tuk.all <- as.data.frame(cbind(round(tuk.B$`p adj`, digits=3), round(tuk.F$`p adj`, digits = 3), round(tuk.Fu$`p adj`, digits = 3), round(tuk.G$`p adj`, digits = 3), round(tuk.T$`p adj`, digits = 3), round(tuk.S$`p adj`, digits = 3), round(tuk.A$`p adj`, digits = 3)))
...
data wrangling and plotting genera stacks
```{r, warning=FALSE, message=FALSE}
...
g.aov.source.signif <- rbind(g.aov.B, g.aov.F, g.aov.E, g.aov.Ce, g.aov.Cl, g.aov.St)
...
g.aov.source.signif$P <- as.character(g.aov.source.signif$P)
g.i <- sapply(g.tuk.all, is.numeric)
g.tuk.all[g.i] <- lapply(g.tuk.all[g.i], as.character)
